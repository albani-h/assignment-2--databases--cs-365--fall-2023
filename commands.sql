...
--get password information from all URLS that have https--
  ...
  SELECT website_name,website_url,first_name,last_name,username,email, time_stamp,account_comment, CAST(AES_DECRYPT(user_password,"key")AS CHAR) AS password FROM account_entry JOIN user_info WHERE (user_info.user_ip = account_entry.user_ip) AND website_url LIKE "https%";
